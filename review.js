...
function renderImages(images) {
    $("imageLayers").innerHTML = images.map((image, index) => `
        <div class="preview-image-layer" style="z-index:${index + 1};left:${Number(image.x) || 50}%;top:${Number(image.y) || 50}%;width:${Number(image.width) || 65}%;height:${Number(image.height) || 65}%;transform:translate(-50%,-50%) rotate(${Number(image.rotation) || 0}deg)">
