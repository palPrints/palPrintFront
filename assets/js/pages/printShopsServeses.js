(function (window, document) {
  ...
  function initialize() {
    ...
    // Account shell behavior matches the dashboard without loading its page logic.
    const mobile = window.matchMedia("(max-width: 760px)");
    const sidebar = document.getElementById("walletSidebar");
    const backdrop = document.getElementById("sidebarBackdrop");
    const menu = document.getElementById("menuButton");
    function syncMenu() {
      const open = mobile.matches ? document.body.classList.contains("sidebar-open") : !document.body.classList.contains("sidebar-collapsed");
      menu.setAttribute("aria-expanded", String(open)); menu.setAttribute("aria-label", open ? "إغلاق القائمة" : "فتح القائمة"); menu.querySelector("i").className = "bi bi-" + (open ? "x-lg" : "list");
      sidebar.inert = !open;
      if (!open) sidebar.setAttribute("aria-hidden", "true"); else sidebar.removeAttribute("aria-hidden");
    }
    function closeMenu() { document.body.classList.remove("sidebar-open"); backdrop.classList.remove("open"); syncMenu(); }
    menu.addEventListener("click", function () { if (mobile.matches) { const open = !document.body.classList.contains("sidebar-open"); document.body.classList.toggle("sidebar-open", open); backdrop.classList.toggle("open", open); } else document.body.classList.toggle("sidebar-collapsed"); syncMenu(); });
    backdrop.addEventListener("click", function () { closeMenu(); menu.focus(); });
    mobile.addEventListener("change", closeMenu);
    const dropdowns = ["account", "notification"];
    function closeDropdowns() { dropdowns.forEach(function (name) { const popup = document.getElementById(name + "Dropdown"); popup.hidden = true; popup.classList.remove("open"); document.getElementById(name + "Button").setAttribute("aria-expanded", "false"); }); }
    dropdowns.forEach(function (name) { document.getElementById(name + "Button").addEventListener("click", function () { const popup = document.getElementById(name + "Dropdown"); const open = popup.hidden; closeDropdowns(); popup.hidden = !open; popup.classList.toggle("open", open); this.setAttribute("aria-expanded", String(open)); if (name === "notification" && open) document.getElementById("notificationCounter").hidden = true; }); });
    document.addEventListener("click", function (event) { if (!event.target.closest(".account-wrap, .notification-wrap")) closeDropdowns(); });
    document.addEventListener("keydown", function (event) { if (event.key === "Escape" && !catalogDialog.open && !settingsDialog.open) { closeDropdowns(); if (mobile.matches && document.body.classList.contains("sidebar-open")) { closeMenu(); menu.focus(); } } });
    document.getElementById("logoutButton").addEventListener("click", function () { if (window.confirm("هل تريد تسجيل الخروج من حساب المطبعة؟")) window.location.href = this.dataset.href; });
    let unread = 0;
    window.addEventListener("palprints:notification", function (event) { const list = document.getElementById("notificationList"); const empty = list.querySelector(".notification-empty"); if (empty) empty.remove(); list.prepend(element("li", "notification-item", event.detail && event.detail.message ? event.detail.message : "لديك إشعار جديد")); unread = document.getElementById("notificationCounter").hidden ? 1 : unread + 1; const counter = document.getElementById("notificationCounter"); counter.textContent = unread > 9 ? "9+" : String(unread); counter.hidden = false; });
    renderProducts(); syncMenu();
  }
