...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function syncMenuState() {
    const open = mobileLayout.matches
  ...
  const notificationButton = document.getElementById("bellButton");
  const notificationMenu = document.getElementById("notificationMenu");
  const notificationDot = notificationButton.querySelector(".dot");
  ...
  notificationButton.addEventListener("click", function (event) {
    event.stopPropagation();
    const open = !notificationMenu.classList.contains("open");
    ...
    if (open) notificationDot.hidden = true;
  });

  document.addEventListener("click", function (event) {
    if (profileMenu.contains(event.target) || notificationMenu.contains(event.target)) return;
    closeProfileMenu();
    closeNotificationMenu();
  });
